...
# Settings
random_seed = 612
n_generations = 3000       # fresh restart from champion_0035, not tied to prior runs' generation count
save_freq = 10
...
config_path = os.path.join(local_dir, "neat_config_selfplay_v8.txt")

# fresh restart, not a checkpoint resume -- population is seeded from
# this genome instead (see seed_and_reset_population() below), and the
# archive starts as just [TrackingPolicy, champion_0035], deliberately
# NOT reloading the weaker 36-42 lineage produced since.
seed_genome_path = os.path.join(local_dir, "neat_run11_full", "champion_0035.pkl")
# total champions ever produced across all prior runs, so this run's new
# champions continue the numbering (0043, 0044, ...) instead of colliding.
starting_champion_count = 42

logdir = "neat_run18_full"
...
def measure_behavior(policy, opponent, n_steps=600):
  ...
  (x_stdev, ball_touches) from the same rollout. x_stdev added after
  champion_0039 (run 14) passed the 20-rollout dethroning score check
  while spending nearly the entire match pinned against its own back
  wall, barely reacting to bx -- the user caught this on screen and
  ...
  return (np.std(xs) if xs else 0.0), touches[0]
...
def run():
  ...
  # Archive starts as just [TrackingPolicy, champion_0035] -- not the
  # weaker 36-42 lineage produced since (see header comment). generation
  # is set to the true historical count so new champions continue the
  # numbering instead of colliding with prior runs' files.
  archive.generation = starting_champion_count
  with open(seed_genome_path, "rb") as f:
    seed_genome = pickle.load(f)
  archive.archive.append(NeatPolicy(seed_genome, config))
  print(f"RESTART: archive = [TrackingPolicy, champion_0035] "
        f"(generation counter starts at {archive.generation})")

  # Fresh population seeded from 128 mutated copies of champion_0035,
  # instead of NEAT's usual random-genome start (see run 12's
  # seed_population(), applied here without run 12's mistake of also
  # mixing in a harsh external opponent pool). Uses
  # reproduction.genome_indexer for fresh, non-colliding genome keys.
  ...
  # hidden-node IDs) can be poisoned by the mutate() calls above too, not
  # just by a checkpoint resume (see run 14's postmortem: it lazily
  # initializes from whichever genome's node_dict is mutated first,
  # which the seeding loop's per-copy mutate() calls can already have
  # made inconsistent across the population by this point). Set it
  # explicitly above every existing node ID before running, always.
  max_node_id = max(
  ...
  print(f"RESTART: seeded population from {seed_genome_path}, "
        f"{len(population.species.species)} species under "
